...
public class TwoPlayerBoardPainter implements IBoardPainter {
	...
	public void paint(PApplet p, ITapBoard board) {
		...
		p.textSize(32);
		p.fill(0xff33DAFD);
		p.text(Integer.toString(board.getScores()[0]), w/2, h/4);
		p.fill(0xffF012BD);
		p.text(Integer.toString(board.getScores()[1]), w/2, 3*h/4);
